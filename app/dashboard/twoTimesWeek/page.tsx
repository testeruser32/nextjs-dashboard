export default function Page() {
  return (
    <div>
      <ul>
        <li>
          <strong>Yoga-Workout:</strong> 2x 🧘
        </li>

        <li>
          <strong>Caminhada (Seg à Sábado):</strong> 1h30 (9km) - 2h (12km)
        </li>
        <li>
          <em>
            [Quando vou caminhar, caminho apenas 1h30, não conto quanto fiquei em
            pé em outras atividades]
          </em>
        </li>

        <li>
          <strong>💡 Idéias 💡</strong>
          <ul>
            <li>Tudo que eu compro que não seja comida, é uma péssima ideia.</li>
            <li>🦷 Escovar com as duas mãos 🦷</li>
          </ul>
        </li>

        <li className="p-10">
          <strong>Anotações gerais:</strong>
          <ul>
            <li>30 fica muito inchado</li>
            <li>
              1 Hasmstring + 🪜 CÁRDIO: ESCADA (20) - 30 - 45 MIN
              <ul>
                <li>
                  Opcional:
                  <ul>
                    <li>1° Pendurar(2 maos apenas, Sem forçar e focar costa) || Voador dorsal 2 à 5x 5 à 10</li>
                    <li>2° Calf Twin (1) - 2x 20 à 40</li>
                    <li>3° Bíceps Twin (2) à 4x 8 à (12) (2x para ir na rua, pessoas gostarem de você)</li>
                    <li>3° plank (3) à 4x 20 - (30) seg (Resfriado)</li>
                  </ul>
                </li>
              </ul>
              (Seg, Qua, Sex e Sab)
            </li>
            <li>
              1 Hasmstring + 🪜 CÁRDIO: ESCADA (10) - 20 MIN (Ter, Qui, Dom)
            </li>
          </ul>
        </li>



        <li className="p-10">
          <strong>Segunda:</strong>
          <ul>
            <li>1° Alongamento (Full body)</li>
            <li>1- Treinar língua 4x 30 seg (não tô ligando p/ nada)</li>
            <li>2- Step up 3 - 4x 15 - (12)</li>
            <li>3- Twin Bíceps 4x12 | 4 - (6) - 8 kg</li>
            <li>4- Elevação lateral 3 - 4x 8 à 12 | 4 - (6) - 8 kg</li> 
            <li>5- Sumô 4x 8 à 12 (surdo)</li>        
            <li>
              6- Pendurar 3 - 4x 20 - 30 seg <em>(2 maos apenas, Sem forçar e focar costa)</em>
            </li>
            <li>
              Opcional:
              <ul>
                <li>7- Calf Twin 2 - 3x 40 - (60)</li>
              </ul>
            </li>
          </ul>
        </li>



        <li className="p-10">
          <strong>Quarta:</strong>
          <ul>
            <li>1° Alongamento (Full body)</li>
            <li>1- Treinar língua 4x 30 seg (não tô ligando p/ nada)</li>
            <li>2- Step up 3 - 4x 15 - (12)</li>
            <li>3- Single leg || (Twin) stiff 3 - 4x 8 à 12 (10 kg)</li>
            <li>4- Calf Twin 2 - 3x 40 - (60)</li>
            <li>5- Sumô 4x 8 à 12 (surdo)</li>
            <li>
              6- Pendurar 3 - 4x 20 - 30 seg <em>(2 maos apenas, Sem forçar e focar costa)</em>
            </li>
            <li>
              Opcional:
              <ul>              
                <li>7- plank (3) à 4x 20 - (30) seg (Resfriado)</li>              
              </ul>
            </li>
          </ul>
        </li>



        <li className="p-10">
          <strong>Sexta:</strong>
          <ul>
            <li>1° Alongamento (Full body)</li>
            <li>1- Step up 3 - 4x 15 - (12)</li>
            <li>2- Adultores 3 - (4)x (8) à 12 reps/seg</li>
            <li>3- Abdultores 3 - (4)x (8) à 12 reps</li>
            <li>4- Twin Bíceps 4x12 | 4 - (6) - 8 kg</li>
            <li>5- Elevação lateral 3 - 4x 8 à 12 | 4 - (6) - 8 kg</li>
            <li>6- Flexão com joelho 1x 15 à 20</li>
            <li>
              Opcional:
              <ul>              
                <li>7- Pendurar 3 - 4x 20 - 30 seg <em>(2 maos apenas, Sem forçar e focar costa)</em></li>              
                <li>8- plank (3) à 4x 20 - (30) seg (Resfriado)</li>              
              </ul>
            </li>
          </ul>
        </li>



        <li className="p-10">
          <strong>Sábado:</strong>
          <ul>
            <li>1° Rotacionar quadril (dentro e fora) 2 - (3)x (10) - 15</li>
            <li>2° Alongamento (Full body)</li>
            <li>1- Treinar língua 4x 30 seg (não tô ligando p/ nada)</li>
            <li>2- Step up 3 - 4x 15 - (12)</li>
            <li>3- Single leg || (Twin) stiff 3 - 4x 8 à 12 (10 kg)</li>
            <li>4- Calf Twin 2 - (3)x 40 - (60)</li>          
            <li>5- Plank 3 - (4)x (30) - 45 seg</li>
            <li>6- Back push up 3 - (4)x (30) - 45 seg</li>
            <li>
              7- Pendurar 3 - 4x 20 - 30 seg <em>(2 maos apenas, Sem forçar e focar costa)</em>
            </li>
            <li>
              3° Liberação miofascial (panturrilha, posterior de coxa, glúteos e
              lombar)
            </li>
          </ul>
        </li>
      </ul>
    </div>
  );
}
